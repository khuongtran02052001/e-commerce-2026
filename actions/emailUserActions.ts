...
export async function createAddressForUser(addressData: CreateAddressData) {
  try {
    ...
    if (addressData.isDefault) {
      const existingAddresses = await client.fetch(`*[_type == "address" && email == $email]`, {
        email: userEmail,
      });
...
export async function updateAddressForUser(addressId: string, addressData: CreateAddressData) {
  try {
    const { userId } = await auth();
    const user = await currentUser();
