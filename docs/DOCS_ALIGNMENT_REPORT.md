...
- `POST /orders` is now fully server-authoritative for pricing:
  ...
  - duplicate items are merged by quantity
  - product existence + stock sufficiency validated
  - `subtotal/tax/shipping/totalPrice` computed from env config:
    - `ORDER_TAX_RATE`
    - `ORDER_SHIPPING_FEE`
...
- `docs/REST_FEATURE_FLOW_FOR_BE.md` (feature-by-feature execution flow)
...
2. `REST_FEATURE_FLOW_FOR_BE.md` for sequence/permission flow
3. Swagger for payload validation
4. This alignment report for understanding legacy docs vs active backend behavior

## 6. Known Gaps To Avoid Confusion

- `/user/*` endpoints:
  - Still exist for compatibility.
  - New FE code should use canonical `/orders`, `/reviews`, `/newsletter`, `/admin` routes.
