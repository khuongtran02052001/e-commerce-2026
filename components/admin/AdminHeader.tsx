...
const AdminHeader = ({ user }: AdminHeaderProps) => {
  ...
  const [notifications, setNotifications] = useState<(NotificationType | LocalNotification)[]>([]);
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      ...
      try {
        ...
        const ordersRes = await fetchService(
          '/admin/orders?limit=1000&sortBy=orderDate&sortOrder=desc',
        );
        const ordersData = await ordersRes.json();
        const ordersList: Order[] = ordersData?.data ?? ordersData?.orders ?? [];
        ...
        // Fetch users count
        const usersRes = await fetchService('/admin/users?limit=1');
        const usersData = await usersRes.json();
        const activeUsers = usersData.totalCount || 0;

        // Fetch notifications from API
        const notificationsRes = await fetchService('/admin/notifications');
          ...
          <Sheet open={isNotificationOpen} onOpenChange={setIsNotificationOpen}>
            ...
            <SheetContent side="right" className="w-full sm:max-w-md p-0">
              <SheetHeader className="p-6 border-b">
                <SheetTitle className="text-xl font-bold text-dark-color">Notifications</SheetTitle>
              </SheetHeader>
                  ...
                  notifications.map((notification) => (
                    ...
                    >
                      ...
                      <div className="flex-1 space-y-1">
                        <div className="font-medium text-dark-color">{notification.title}</div>
                        {notification.description && (
                          <div className="text-sm text-light-color">{notification.description}</div>
                        )}
                        <div className="text-xs text-light-color">{notification.time}</div>
                      </div>
                    </div>
                  ))
                )}
