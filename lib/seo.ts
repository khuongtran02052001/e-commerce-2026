...
export function generateProductMetadata(product: any): Metadata {
  ...
  const imageUrl = product.images?.[0] ? urlFor(product.images[0]).url() : '/og-image.jpg';
 ...
 */
export function generateCategoryMetadata(category: Category, productCount: number = 0): Metadata {
  ...
  const imageUrl = category.image ? urlFor(category.image).url() : '/og-image.jpg';
