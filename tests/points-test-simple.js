...
function calculateLoyaltyPoints(completedOrders) {
  const milestones = Math.floor(completedOrders / LOYALTY_POINTS_ORDER_THRESHOLD);
...
testOrders.forEach((order, index) => {
  ...
  console.log(`   Order ${index + 1}: $${order.totalAmount} → ${points} reward points`);
});
...
for (let orders = 1; orders <= 15; orders++) {
  ...
  if (orders % 5 === 0 || orders <= 7) {
    console.log(`   ${orders} completed orders → Total: ${points} loyalty points`);
  }
}
...
scenarios.forEach((scenario, index) => {
  ...
  const newLoyaltyPoints = calculateLoyaltyPoints(scenario.completedOrders + 1) - loyaltyPoints;

  console.log(`   Scenario ${index + 1}: ${scenario.description}`);
  console.log(`     Order: $${scenario.orderTotal} → ${rewardPoints} reward points`);
  console.log(
