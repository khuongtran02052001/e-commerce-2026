...
export const userType = defineType({
  ...
  fields: [
          ...
          preview: {
            ...
            prepare(select) {
              ...
              return {
                ...
                subtitle: `${description} • ${new Date(createdAt).toLocaleDateString()}`,
              };
            },
          },
        }),
          ...
          preview: {
            ...
            prepare(select) {
              ...
              return {
                ...
                subtitle: `${method} • ${new Date(requestedAt).toLocaleDateString()}`,
              };
            },
          },
        }),
    ...
    defineField({
      ...
      type: 'boolean',
      initialValue: false,
      description: 'Whether the user has premium account status (formerly Active Account)',
    }),
    defineField({
      name: 'premiumStatus',
    ...
    defineField({
      ...
      title: 'Rejection Reason',
      type: 'text',
      description: 'Reason for rejection (for both premium and business applications)',
    }),
    defineField({
      name: 'isBusiness',
    ...
    defineField({
      ...
      hidden: ({ document }) => !document?.isEmployee || document?.employeeStatus !== 'suspended',
    }),
    defineField({
  ...
  preview: {
    ...
    prepare(select) {
      const { firstName, lastName, email, image, isActive, isAdmin, isEmployee, employeeRole } =
        select;
