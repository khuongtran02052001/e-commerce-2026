...
export const reviewType = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'string',
      validation: (Rule) =>
        Rule.required().min(5).max(100).error('Title must be between 5 and 100 characters'),
  ...
  preview: {
    ...
    prepare(selection) {
      const { title, rating, status, userName, userLastName, productName } = selection;
      const stars = '★'.repeat(rating || 0) + '☆'.repeat(5 - (rating || 0));
      const reviewer = userName && userLastName ? `${userName} ${userLastName}` : 'Unknown User';

      return {
        title: title || 'Untitled Review',
        subtitle: `${stars} - ${reviewer} | ${productName || 'Unknown Product'}`,
