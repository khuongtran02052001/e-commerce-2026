...
export default function UserSettingsPage() {
  ...
  const handleSettingChange = async (key: string, value: boolean) => {
    try {
      const response = await fetchService('/user/settings', {
        method: 'PATCH',
  ...
  const handleExportData = async () => {
    try {
      const response = await fetchService('/user/export-data');
      if (response.ok) {
  ...
  const handleDeleteAccount = async () => {
    if (
      ...
      try {
        const response = await fetchService('/user/delete-account', { method: 'DELETE' });
