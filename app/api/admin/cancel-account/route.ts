...
export async function POST(req: NextRequest) {
  try {
    ...
    const currentStatus =
      type === 'premium' ? existingUser.premiumStatus : existingUser.businessStatus;

    if (currentStatus !== 'active') {
      return NextResponse.json({ error: `${type} account is not active` }, { status: 400 });
    }
    ...
    const result = await backendClient.transaction().patch(accountId, { set: updateData }).commit();
