...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, phone, address, city, state, zip, country, default: isDefault, type } = body;
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { _id, name, phone, address, city, state, zip, country, default: isDefault, type } = body;
  ...
  try {
    ...
    if (existingAddress.default) {
      ...
      if (remainingAddresses) {
        await client.patch(remainingAddresses._id).set({ default: true }).commit();
      }
    }
