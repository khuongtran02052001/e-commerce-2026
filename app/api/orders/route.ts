...
export const POST = async (request: NextRequest) => {
  try {
    ...
    const { items, shippingAddress, paymentMethod, totalAmount, subtotal, shipping, tax } = reqBody;
    ...
    const userPhone = user.phoneNumbers?.[0]?.phoneNumber || shippingAddress.phone || '';
